class InicializaciaPola():
    ...
    def vytvorPoleHran(self):
        pocetH = self.pocetHran//2
        pole = [[0 for x in range(3)] for y in range(pocetH * 2 + 1)]
        with open(self.cestaKSuboru, 'r') as f:
            for index, riadok in enumerate(f):
                r = riadok.split()
                for index2, cislo in enumerate(r):
                    pole[index + 1][index2] = int(cislo)
                pole[index + pocetH + 1][0] = pole[index+1][1]
                pole[index + pocetH + 1][1] = pole[index+1][0]
    ...
    def vytvorPoleS(self):
        pole = [[0 for x in range(2)]
                for y in range(self.pocetVrcholov + 2)]
        hpole = self.poleHran
