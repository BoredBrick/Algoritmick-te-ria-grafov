...
class Kruskalov():
    ...
    def __init__(self, cesta):
        ...
        # zistim pocet vrcholov z druheho stlpca
        maxi = max(l[1] for l in self.H)
        # pocet vrcholov je vacsie zislo z prveho alebo druheho stlpca
        self.pocetV = self.pocetV if self.pocetV > maxi else maxi
        # zoradim hrany podla ich ceny od najmensej
        self.H.sort(key=lambda x: x[2])
        print(self.H)
        self.k = {}

    def kruskalov(self):
        # tu zapisujem kostru grafu
        k = {}
        postupnostHran = []
        ...
        p.pop(0)
        # pocet vybranych hran
        pocetVr = 0
        # cena kostry grafu
        cena = 0
        # prechadzam, pokial mam menej vybratych hran ako je pocet vrcholov alebo pokial postupnost p nie je prazdna
        while (len(p) != 0) and (pocetVr < self.pocetV - 1):
            hrana = p[0].split(",")
            u = int(hrana[0])
            v = int(hrana[1])
            p.pop(0)
            # ak nemaju rovnaky komponent, tak pripocitam cenu hrany a nastavim komponenty vsetkym, ktorych sa to tyka
            if k[u] != k[v]:
                ...
                for vrchol in k:
                    if k[vrchol] == kv:
                        k[vrchol] = k[u]
        # zistim pocet komponentov a ocislujem ich postupne od 1
        komponenty = []
        for i in range(1, self.pocetV + 1):
